...
const Motion=(()=>{
 ...
 function page(from,to){pageAnimation?.cancel();const el=document.querySelector('#app-content>.page');if(!enabled()||!el)return;pageAnimation=animate(el,[{opacity:.5,transform:'translateY(10px)'},{opacity:1,transform:'translateY(0)'}],{duration:300});}
 ...
 function success(button){if(!button||!enabled())return;animate(button,[{scale:'.98'},{scale:'1.015',offset:.6},{scale:'1'}],{duration:260});}
